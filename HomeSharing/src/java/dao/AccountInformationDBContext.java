...
public class AccountInformationDBContext extends DBContext<AccountInformation> {
    ...
    public AccountInformation get(AccountInformation model) {
        ...
        try {
            ...
            accountInformation.setPhone(rs.getString(9));
            connection.commit();
            stm.close();
            connection.close();
...
//Get data information from database 
    ...
    public AccountInformation viewProfile(String username) {
        try {
            connection.setAutoCommit(false);
    ...
    public AccountView getGoogleEmail(String email) {
        ...
        try {
            String sql = "select username, [password],email from AccountInformation ai\n"
            ...
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
                accountView.setUsername(rs.getString(1));
                accountView.setPassword(rs.getString(2));
                accountView.setEmail(rs.getString(3));
            }
        } catch (SQLException ex) {
            Logger.getLogger(AccountInformationDBContext.class.getName()).log(Level.SEVERE, null, ex);
        }
